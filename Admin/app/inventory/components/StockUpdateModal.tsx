...
interface StockUpdateModalProps {
  ...
  onClose: () => void;
  onSave: (id: string, data: { stock?: number; variants?: { colorName: string; stock: number }[] } | number) => Promise<void>;
  saving: boolean;
}
...
export default function StockUpdateModal({
  ...
  const [variantsState, setVariantsState] = useState(
    hasVariants
      ? product.colorVariants!.map((cv) => ({ colorName: cv.name, stock: cv.stock, hex: cv.hex }))
      : []
  );

  const handleSubmit = async () => {
    if (hasVariants) {
      await onSave(product._id, { variants: variantsState.map(v => ({ colorName: v.colorName, stock: v.stock })) });
    } else {
  ...
  const totalVariantStock = hasVariants
    ? variantsState.reduce((sum, v) => sum + v.stock, 0)
    : stock;
  const diff = hasVariants ? totalVariantStock - product.stock : stock - product.stock;

  const hasChanges = hasVariants
    ? variantsState.some((v, i) => v.stock !== product.colorVariants![i].stock)
    : stock !== product.stock;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      {/* Modal */}
      <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-md mx-4 overflow-hidden animate-[scaleIn_0.2s_ease-out] flex flex-col max-h-[90vh]">
        {/* Header */}
          ...
          <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
            ...
            <div className="flex flex-col min-w-0">
              ...
              <span className="text-xs text-gray-500">
                {product.category} · Current total stock: {product.stock}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="px-6 pb-4 overflow-y-auto min-h-0 flex-1">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                {variantsState.map((variant, index) => (
                  <div key={variant.colorName} className="flex items-center justify-between bg-white border border-gray-100 rounded-xl p-3 shadow-sm">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <button
                        onClick={() => {
                          ...
                          next[index].stock = Math.max(0, next[index].stock - 1);
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          const next = [...variantsState];
                          next[index].stock = Math.max(0, parseInt(e.target.value) || 0);
                          setVariantsState(next);
                        }}
                        className="w-14 text-center text-lg font-bold text-gray-900 border-b-2 border-transparent focus:border-[#a1005b] outline-none transition-colors bg-transparent"
                      />
                      <button
                        onClick={() => {
                          const next = [...variantsState];
          ...
          {diff !== 0 && (
            <div className="mt-6 text-center">
              ...
              >
                {diff > 0 ? `+${diff}` : diff} total from current
              </span>
            </div>
          )}
        </div>
        ...
        {/* Actions */}
        <div className="flex items-center gap-3 p-6 pt-4 border-t border-gray-100 shrink-0">
          <button
            onClick={onClose}
            disabled={saving}
